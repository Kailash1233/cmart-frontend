...
const SimpleProduct = ({ product }: { product: ProductType }) => {
  ...
  return (
    <div className="d-flex w-100 gap-3 h-25 mb-3 bg-white">
      ...
      <div className="w-75">
        <Link to={"/product/" + product.id}>
          <span className="product-name my-2 fw-bold text-dark">
            {product.name}
          </span>
        </Link>
        <div className="d-flex justify-content-between">
          ...
          <div
            className="close-btn me-4 fd-bg-primary text-white text-center rounded-5 cursor-pointer"
            style={{ width: "30px", height: "30px" }}
            onClick={() => dispatch(deleteProductInCart(product))}
          >
            <span>
              <i className="bi bi-x" style={{ lineHeight: "30px" }}></i>
            </span>
          </div>
        </div>
      </div>
